...
public class MakeCashRegister {
           
    private static void function(double PP, double CH){
        if(CH < PP){
        ...
        double[] leftovers = new double[]{0.01, 0.05, 0.1, 0.25, 0.5, 1, 2, 5, 10, 20, 50, 100};
        String[] leftovers_string = new String[]{"PENNY", "NICKEL", "DIME", "QUARTER", "HALF DOLLAR", "ONE", "TWO", "FIVE", "TEN",
        "TWENTY", "FIFTY", "HUNDRED"};
        int index = 11;
        while(change != 0){
            System.out.println(change + " " + index);
            if(change >= leftovers[index]){
                result.add(leftovers_string[index]);
                change-=leftovers[index];
                change = (Math.round(change*100.0) / 100.0);
            } else{
                index--;
            }
        ...
        CompareByLength comp = new CompareByLength();
